...
fn test_is_retryable_error() {
    assert!(GrpcClientPool::is_retryable_error(&GrpcError::Timeout("test".to_string())));
    assert!(GrpcClientPool::is_retryable_error(&GrpcError::ConnectionError("test".to_string())));
    assert!(GrpcClientPool::is_retryable_error(&GrpcError::CallFailed("Unavailable".to_string())));
    assert!(!GrpcClientPool::is_retryable_error(&GrpcError::ServiceNotFound("test".to_string())));
    assert!(!GrpcClientPool::is_retryable_error(&GrpcError::InvalidConfig("test".to_string())));
}
